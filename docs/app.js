(() => {
  ...
  async function connectDownload() {
    ...
    const assetName = config.assetName || "OfferPilot_v53_Setup.exe";
    ...
    try {
      ...
      const asset = assets.find((item) => item.name === assetName && item.state === "uploaded")
        || assets.find((item) => item.name === config.fallbackAssetName && item.state === "uploaded");
      if (!asset) {
      ...
      const isInstaller = /\.exe$/i.test(asset.name);
      const installHint = $("#install-hint");
      if (installHint) installHint.textContent = isInstaller
        ? "双击安装，桌面直接打开。"
        : "解压后打开包内的安装程序。";
      const size = Number(asset.size) > 0 ? ` · ${(Number(asset.size) / 1024 / 1024).toFixed(1)} MB` : "";
      status.textContent = `${config.version || "v53"} · Windows 64 位${size}`;
